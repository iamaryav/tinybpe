"""
This class implements GPT-4 tokenizer using our RegexTokenizer
It uses pretrained tokenizer present in tiktoken "c1100k_base"
I mostly followed minbpe repo code

Note: In future, I might implement a new tokenizer, but currently just learning
...
def recover_merges(mergeable_ranks):
    # Recovering original merges from byte sequence in their merge state
    ...
    for token, rank in mergeable_ranks.items():
        ...
        assert len(pair) == 2
        # which two token was merged
        ix0 = mergeable_ranks[pair[0]] 
        ix1 = mergeable_ranks[pair[1]]
        # rank: is the new token that was assigned to ix0, ix1
        # merges: {(ix0, ix1) -> rank}
        merges[(ix0, ix1)] = rank
...
# Just copied from minbpe repo
# trying to understand with tests
